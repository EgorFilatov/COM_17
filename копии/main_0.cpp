...
#define UART_TX_PERIOD_MS 		3000	//Период выдачи по UART (мс)
...
void HAL_UART_TxCpltCallback(UART_HandleTypeDef *huart) {
	...
	if (i != numOfSpiDevices) {
		...
		uartTxState = BUSY;
		// Передача данных по uart
		HAL_UART_Transmit_DMA(&huart5, spiDevice[i].getRxBuffPtr(currentBuffIndex), 10);
	} else {
		SpiDevice::setRxBuffState(currentBuffIndex, READY);
		uartTxState = READY;
	}
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
	uartRxBuffState[currentUartRxBuffIndex ^ 1] = READY;
	uartRxState = READY;
}
...
void processUartRxBuff() {
	...
	uint16_t checksum { 0 };
	for (uint8_t i = 0; i < 8; ++i) {
		checksum += uartRxBuffPtr[currentBuffIndex][i];
	}
	uartTxBuffPtr[currentBuffIndex][8] = (uint8_t) checksum;
	uartTxBuffPtr[currentBuffIndex][9] = (uint8_t) (checksum >> 8);
	if ((uint8_t) checksum == uartRxBuffPtr[currentUartRxBuffIndex][8] &&
		(uint8_t) (checksum >> 8) == uartRxBuffPtr[currentBuffIndex][9]) {
		SpiDevice::setTxBuffState(currentBuffIndex, BUSY);
